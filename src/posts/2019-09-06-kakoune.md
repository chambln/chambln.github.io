...
![Screencast to demonstrate Kakoune's `:man` command wrapper.]
...
stand by its decision not to complect its language with flow control and
...
Same goes for `v` and the 'view' menu, which is for adjusting your
'view port' into the buffer.
...
![Screencast: Column-wise alignment of a markdown table.]

### Narrowing

Still think multiple selection are a gimmick? So did I. Until I tried
some of the narrowing mechanisms. What do I mean by that? Well, let's
say you want to replace all occurrences of 'that' with 'this' inside a
...
to select each occurrence of 'that'.[^6] Now you have multiple
selections. Finally you'd type

    cthis<esc>

to change each into 'this'.

![Screencast: Replacing instances of a pattern in a paragraph.]
...
program-yielding text-processing glue. The idea is that you make your
...
![Screencast: Evaluating a Python expression in Kakoune.]
...
of file you're editing? You can do that using the WinSetOption hook:
    ...
    hook <scope> WinSetOption 'filetype=(ruby)' <command>

The command can be any list of commands. So to set a few options that
would be something like this:

``` {.ruby}
hook <scope> WinSetOption 'filetype=(css)' %{
...
[^8]: GNU readline(1) is the line-editing library used by countless
  ...
  [Screencast to demonstrate Kakoune's `:man` command wrapper.]: media/kak-man.webm
  ...
  [Screencast: Column-wise alignment of a markdown table.]: media/kak-align.webm
  [Screencast: Replacing instances of a pattern in a paragraph.]: media/kak-sthatcthis.webm
  [pandoc(1)]: https://pandoc.org/
  [Screencast: Evaluating a Python expression in Kakoune.]: ./media/kak-python.webm
